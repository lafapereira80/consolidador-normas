...
def analisar_lote_arquivos(arquivos, key, provedor, thinking_level="medium", dpi_ocr=1.5, max_paginas_ocr=None, progresso=None):
    ...
    # NOVO: Verificação de arquivo único que é alteradora sem base
    if len(arquivos) == 1 and len(triagem_dados) > 0:
        unico = triagem_dados[0]
        if unico.get('tipo') == 'Alteradora':
            base_encontrada = _localizar_base_no_banco(unico.get('ato_base_referenciado_tipo'), unico.get('ato_base_referenciado_numero'))
            if not base_encontrada:
                referencias_pendentes = [{
                    "ato_referenciado_tipo": unico.get('ato_base_referenciado_tipo') or 'Desconhecido',
                    "ato_referenciado_numero": unico.get('ato_base_referenciado_numero') or 'Desconhecido',
                    "arquivos_alteradores": [unico['nome_arquivo_upload']],
                }]
                return {
                    "consolidacoes_geradas": [],
