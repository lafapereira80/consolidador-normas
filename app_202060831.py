...
st.caption("Aceita Leis, Decretos, Resoluções, Enunciados, Portarias e demais atos normativos, em PDF. Envie um ato por vez.")
...
# Toolbar explícito: o pacote streamlit-quill está sem novas versões há mais de
# 12 meses (projeto praticamente abandonado), então nunca confiamos no toolbar
# padrão dele — fixamos aqui exatamente os formatos que o sistema depende
# (negrito, itálico, riscado e cor), garantindo que fiquem sempre disponíveis
# independentemente do que o pacote definir por padrão.
_QUILL_TOOLBAR = [
...
def editor_rico(value, key):
    """Editor de texto rico usado em todo o app. Tenta o Quill (streamlit-quill)
    com toolbar fixo; se o componente falhar ao carregar (risco real dado que o
    pacote está sem manutenção ativa), cai automaticamente para um campo de
    texto simples em HTML, sem derrubar a página."""
    try:
...
class Consolidacao(BaseModel):
    ...
    ato_base_referenciado_tipo: Optional[str] = Field(default=None, description="Preencha SOMENTE se este documento, pelo seu próprio texto (ementa/preâmbulo/artigos), declarar que altera ou revoga OUTRO ato normativo específico (ex.: 'Altera a Portaria nº 130/PGJM...'). Informe o tipo do ato referenciado (ex.: 'PORTARIA'). Deixe vazio/null se este documento não altera nenhum outro ato (é ele mesmo um ato original).")
    ato_base_referenciado_numero: Optional[str] = Field(default=None, description="O número/identificador do ato referenciado em ato_base_referenciado_tipo (ex.: '130/PGJM'). Deixe vazio/null se não houver referência.")
...
# =====================================================================
# FLUXO NOVO: leitura de UM ato por vez -> editor -> salvar -> analisar
# contra os atos já cadastrados (base formal OU avulsos ainda pendentes).
# =====================================================================

def estruturar_documento_unico(arquivo, key, provedor):
    """Lê UM arquivo e o estrutura no padrão do sistema (ementa/preâmbulo/
    dispositivos, com <b>/<i>/tabelas fielmente preservados), sem aplicar
    nenhuma alteração/revogação ainda — é o próprio ato, tal como está."""
    conteudo = extrair_conteudo_multimodal(arquivo.getvalue(), arquivo.name)
    memoria_aprendida = resgatar_memoria()
    prompt = [
        "Documento a estruturar:",
        *conteudo,
        "Estruture este documento normativo (Lei, Decreto, Resolução, Portaria, Enunciado etc.) "
        "seguindo rigorosamente o padrão do sistema: separe ementa e preâmbulo, mapeie cada "
        "dispositivo (capítulo, artigo, parágrafo, parágrafo único, inciso, alínea, anexo, tabela) "
        "preservando <b>/<i>/tabelas com fidelidade absoluta. Este é o texto ORIGINAL — preencha "
        "'texto_principal_alterada' e 'texto_principal_consolidada' com o MESMO texto (idêntico, sem "
        "riscos), pois ainda não há nenhuma alteração aplicada. Some isso: se o próprio texto deste "
        "documento (ementa/preâmbulo/artigos) declarar que ele altera ou revoga OUTRO ato normativo "
        "(ex.: 'Altera a Portaria nº 130/PGJM...'), preencha ato_base_referenciado_tipo e "
        "ato_base_referenciado_numero com o ato referenciado; senão deixe ambos vazios/null."
        + memoria_aprendida,
    ]
    resp = executar_com_fallback(key, prompt, Consolidacao, provedor)
    doc = json.loads(resp.text)
    doc["arquivos_originais_identificados"] = [arquivo.name]
    doc["arquivos_alteradores_identificados"] = []
    doc["normas_alteradoras"] = []
    return doc

def salvar_ato_avulso(doc):
    """Upsert do ato lido/editado na tabela de estágio (antes de saber se é
    base ou derivativo de algo já cadastrado)."""
    if not supabase: return False, "Sem conexão com o banco."
    try:
        base = doc["norma_base"]
        data_iso = converter_para_iso(base.get("data_assinatura"))
        supabase.table("atos_processados").upsert({
            "tipo_documento": base["tipo_documento"], "numero_documento": base["numero_documento"],
            "orgao_emissor": base.get("orgao_emissor"), "data_assinatura": data_iso,
            "nome_padronizado": base["nome_padronizado"],
            "ato_base_referenciado_tipo": doc.get("ato_base_referenciado_tipo"),
            "ato_base_referenciado_numero": doc.get("ato_base_referenciado_numero"),
            "documento_estruturado_json": doc,
        }, on_conflict="nome_padronizado").execute()
        return True, None
    except Exception as e:
        return False, str(e)

def _buscar_ato_por_referencia(tipo_ref, numero_ref):
    """Procura um ato (formalmente base OU ainda avulso/pendente) que
    corresponda ao tipo/número referenciado. Retorna (origem, registro) onde
    origem é 'base', 'avulso' ou None."""
    if not supabase or not numero_ref or not str(numero_ref).strip():
        return None, None
    numero_limpo = str(numero_ref).strip()
    try:
        res_base = supabase.table("portarias_base").select(
            "id, nome_padronizado, tipo_documento, numero_documento, documento_consolidado_json"
        ).ilike("numero_documento", f"%{numero_limpo}%").execute()
        if res_base.data:
            if tipo_ref:
                for c in res_base.data:
                    if str(c.get("tipo_documento", "")).strip().lower() == str(tipo_ref).strip().lower():
                        return "base", c
            return "base", res_base.data[0]
    except Exception:
        pass
    try:
        res_avulso = supabase.table("atos_processados").select(
            "id, nome_padronizado, tipo_documento, numero_documento, documento_estruturado_json"
        ).ilike("numero_documento", f"%{numero_limpo}%").execute()
        if res_avulso.data:
            if tipo_ref:
                for c in res_avulso.data:
                    if str(c.get("tipo_documento", "")).strip().lower() == str(tipo_ref).strip().lower():
                        return "avulso", c
            return "avulso", res_avulso.data[0]
    except Exception:
        pass
    return None, None

def _buscar_avulso_que_referencia(numero_documento_atual):
    """Busca, entre os atos avulsos pendentes, algum que declare alterar/
    revogar o ato atual (relação inversa: o ato atual é a base de outro já
    lido e salvo, mas ainda não classificado)."""
    if not supabase or not numero_documento_atual:
        return None
    try:
        res = supabase.table("atos_processados").select(
            "id, nome_padronizado, numero_documento, ato_base_referenciado_numero, documento_estruturado_json"
        ).execute()
        numero_limpo = str(numero_documento_atual).strip().lower()
        for row in (res.data or []):
            ref = str(row.get("ato_base_referenciado_numero") or "").strip().lower()
            if ref and (ref in numero_limpo or numero_limpo in ref):
                return row
    except Exception:
        pass
    return None

def _aplicar_ato_sobre_estado(key, provedor, estado_json_atual, ato_novo_dict, ordinal, total, memoria_aprendida):
    """Aplica UM ato alterador/revogador (já estruturado em JSON, vindo do
    editor avulso) sobre o estado atual (JSON) de uma cadeia base+derivadas."""
    conteudo = [
        f"ESTADO ATUAL (JSON):\n{estado_json_atual}",
        f"ATO ALTERADOR/REVOGADOR Nº {ordinal} DE {total} A SER APLICADO, EM ORDEM CRONOLÓGICA DO MAIS "
        f"ANTIGO PARA O MAIS NOVO (já estruturado, revisado pelo usuário no editor — use como fonte de "
        f"verdade do seu conteúdo):\n{json.dumps(ato_novo_dict)}",
        f"""
        Aplique o ato alterador/revogador acima cruzando detalhadamente com o estado atual.
        Obrigatório: siga EXATAMENTE o formato de citação e a estrutura de parágrafos definidos nas
        regras do sistema (item 4) — "(Alterada pelo Art. N da TIPO Nº NÚMERO/ANO - SIGLA)" na linha
        riscada, "(Redação dada pelo Art. N da TIPO Nº NÚMERO/ANO - SIGLA)" na linha nova, ou "(Revogado
        pelo Art. N da TIPO Nº NÚMERO/ANO - SIGLA)" para revogação (sem repetir linha). Se um dispositivo
        já havia sido alterado antes (texto já contém uma nota "(Redação dada pelo ...)" anterior) e agora
        está sendo alterado de novo, risque a redação anterior (a que estava vigente) e acrescente a nova
        redação com sua própria nota — preserve a nota antiga junto ao trecho agora riscado, não a apague,
        para manter o histórico completo de alterações desse dispositivo do mais antigo para o mais novo.
        Preserve <b>/<i> e tabelas ([TABELA]...[/TABELA]) com fidelidade absoluta, redesenhando a tabela
        inteira quando alterada.
        {memoria_aprendida}
        """,
    ]
    resp = executar_com_fallback(key, conteudo, Consolidacao, provedor)
    return json.loads(resp.text)

def analisar_ato_avulso_contra_cadastrados(doc, key, provedor):
    """Pega o ato atualmente em tela (já lido/editado) e confere contra os
    atos já cadastrados (formalmente base, ou ainda avulsos/pendentes) em
    busca de relação de derivação, nos dois sentidos. Se encontrar, aplica a
    cascata (a partir do que já estiver salvo) e devolve o resultado pronto
    para revisão/exportação — sem persistir ainda (isso é feito à parte)."""
    mensagens = []
    memoria_aprendida = resgatar_memoria()
    base_meta = doc["norma_base"]
    tipo_ref = doc.get("ato_base_referenciado_tipo")
    numero_ref = doc.get("ato_base_referenciado_numero")

    if numero_ref:
        origem, achado = _buscar_ato_por_referencia(tipo_ref, numero_ref)
        if origem == "base":
            nome_base = achado["nome_padronizado"]
            estado_json_atual, ja_processadas = _consultar_estado_e_historico(nome_base)
            if not estado_json_atual:
                mensagens.append(("warning", f"⚠️ '{nome_base}' foi localizado no banco, mas sem conteúdo consolidado salvo."))
                return None, mensagens
            if base_meta["nome_padronizado"].lower() in {j.lower() for j in ja_processadas}:
                mensagens.append(("warning", f"⚠️ '{base_meta['nome_padronizado']}' já havia sido aplicado anteriormente a '{nome_base}' — não será reaplicado."))
                return json.loads(estado_json_atual), mensagens
            detalhe = f" ({', '.join(ja_processadas)})" if ja_processadas else ""
            mensagens.append(("info", f"🧠 '{nome_base}' já possui histórico no banco: {len(ja_processadas)} alteração(ões) anterior(es){detalhe}. Aplicando '{base_meta['nome_padronizado']}' em seguida, na ordem cronológica."))
            resultado = _aplicar_ato_sobre_estado(key, provedor, estado_json_atual, doc, len(ja_processadas) + 1, len(ja_processadas) + 1, memoria_aprendida)
            resultado["_base_id_existente"] = achado["id"]
            return resultado, mensagens
        elif origem == "avulso":
            mensagens.append(("info", f"📎 '{base_meta['nome_padronizado']}' altera '{achado['nome_padronizado']}', que também está salvo como ato avulso (ainda sem cadeia formal). Criando a cadeia base + primeira alteração agora."))
            estado_json_atual = json.dumps(achado["documento_estruturado_json"])
            resultado = _aplicar_ato_sobre_estado(key, provedor, estado_json_atual, doc, 1, 1, memoria_aprendida)
            resultado["_avulso_base_id"] = achado["id"]
            return resultado, mensagens

    avulso_dependente = _buscar_avulso_que_referencia(base_meta["numero_documento"])
    if avulso_dependente:
        mensagens.append(("info", f"📎 O ato avulso '{avulso_dependente['nome_padronizado']}' já cadastrado declara alterar/revogar '{base_meta['nome_padronizado']}'. Criando a cadeia base + primeira alteração agora."))
        estado_json_atual = json.dumps(doc)
        ato_alterador = avulso_dependente["documento_estruturado_json"]
        resultado = _aplicar_ato_sobre_estado(key, provedor, estado_json_atual, ato_alterador, 1, 1, memoria_aprendida)
        resultado["_avulso_derivado_id"] = avulso_dependente["id"]
        return resultado, mensagens

    mensagens.append(("info", "ℹ️ Nenhuma relação de derivação encontrada entre este ato e os atos já cadastrados no banco."))
    return None, mensagens

def salvar_cascata_e_vincular(cons):
    """Persiste o resultado da cascata (base + alteradoras + JSON consolidado)
    e marca os atos avulsos envolvidos como vinculados à base formal."""
    ok = salvar_no_supabase(cons, None)
    if not ok or not supabase:
        return ok
    try:
        base_nome = cons["norma_base"]["nome_padronizado"]
        res = supabase.table("portarias_base").select("id").eq("nome_padronizado", base_nome).execute()
        if not res.data:
            return ok
        base_id = res.data[0]["id"]
        nomes_envolvidos = [cons["norma_base"]["nome_padronizado"]] + [a["nome_padronizado"] for a in cons.get("normas_alteradoras", [])]
        for nome in nomes_envolvidos:
            supabase.table("atos_processados").update({"vinculado_a_base_id": base_id}).eq("nome_padronizado", nome).execute()
    except Exception:
        pass
    return ok
...
# ----------------- FRONT-END: LEITURA DE UM ATO POR VEZ -----------------
def renderizar_editor_documento(cons, key_prefix):
    cons['titulo_portaria'] = st.text_input("Título do Ato Normativo", cons.get('titulo_portaria', ''), key=f"titulo_{key_prefix}")

    st.markdown("**Ementa**")
    val_ementa = ia_para_editor(cons.get('ementa', ''))
    ementa_editada = editor_rico(value=val_ementa, key=f"q_ementa_{key_prefix}")
    ...
    preambulo_editado = editor_rico(value=val_preambulo, key=f"q_preambulo_{key_prefix}")
    ...
    for j, disp in enumerate(cons.get("dispositivos", [])):
        ...
        with c_alt:
            ...
            alt_editada = editor_rico(value=val_alt, key=f"q_alt_{key_prefix}_{j}")
        ...
        with c_cons:
            ...
            cons_editada = editor_rico(value=val_cons, key=f"q_cons_{key_prefix}_{j}")
        ...
        nota_editada = st.text_input("Nota", value=disp.get('nota_remissiva', ''), key=f"nota_{key_prefix}_{j}", label_visibility="collapsed")
        ...
        if disp.get('is_tabela'):
            ...
            with t_alt:
                tab_alt_edit = st.data_editor(disp.get('tabela_alterada') or [[""]], key=f"tab_alt_{key_prefix}_{j}", num_rows="dynamic", use_container_width=True)
                disp['tabela_alterada'] = tab_alt_edit if isinstance(tab_alt_edit, list) else disp.get('tabela_alterada')
                pos_alt = st.text_area("Texto após a tabela (Alterada)", value=disp.get('texto_pos_tabela_alterada') or "", key=f"pos_alt_{key_prefix}_{j}")
                disp['texto_pos_tabela_alterada'] = pos_alt
            with t_cons:
                tab_cons_edit = st.data_editor(disp.get('tabela_consolidada') or [[""]], key=f"tab_cons_{key_prefix}_{j}", num_rows="dynamic", use_container_width=True)
                disp['tabela_consolidada'] = tab_cons_edit if isinstance(tab_cons_edit, list) else disp.get('tabela_consolidada')
                pos_cons = st.text_area("Texto após a tabela (Consolidada)", value=disp.get('texto_pos_tabela_consolidada') or "", key=f"pos_cons_{key_prefix}_{j}")
                disp['texto_pos_tabela_consolidada'] = pos_cons

def renderizar_exportacao(cons, key_prefix):
    st.markdown("### 📥 Opções de Exportação")
    nome_arquivo_base = cons['norma_base']['nome_padronizado'].replace(' ', '_').replace('/', '-')
    c_html, c_pdf, c_docx = st.columns(3)

    try:
        html_alt = gerar_html_dinamico(cons, "alterada")
        html_cons = gerar_html_dinamico(cons, "consolidada")
        c_html.download_button("🌐 Baixar HTML (Alterada)", data=html_alt, file_name=f"{nome_arquivo_base}_Alt.html", mime="text/html", key=f"ha_{key_prefix}")
        c_html.download_button("🌐 Baixar HTML (Consolidada)", data=html_cons, file_name=f"{nome_arquivo_base}_Cons.html", mime="text/html", key=f"hc_{key_prefix}")
    except Exception as e:
    ...
    try:
        ...
        c_pdf.download_button("📄 Baixar PDF (Alterada)", data=pdf_alt, file_name=f"{nome_arquivo_base}_Alt.pdf", mime="application/pdf", key=f"pa_{key_prefix}")
        c_pdf.download_button("📄 Baixar PDF (Consolidada)", data=pdf_cons, file_name=f"{nome_arquivo_base}_Cons.pdf", mime="application/pdf", key=f"pc_{key_prefix}")
    except Exception as e:
    ...
    try:
        ...
        c_docx.download_button("📝 Baixar DOCX (Alterada)", data=docx_alt, file_name=f"{nome_arquivo_base}_Alt.docx", mime="application/vnd.openxmlformats", key=f"da_{key_prefix}")
        c_docx.download_button("📝 Baixar DOCX (Consolidada)", data=docx_cons, file_name=f"{nome_arquivo_base}_Cons.docx", mime="application/vnd.openxmlformats", key=f"dc_{key_prefix}")
    except Exception as e:
        c_docx.error(f"Falha ao gerar DOCX: {e}")

for chave, inicial in [("ato_avulso", None), ("ato_avulso_original", None), ("resultado_cascata", None), ("resultado_cascata_original", None)]:
    if chave not in st.session_state: st.session_state[chave] = inicial

st.markdown("<br>", unsafe_allow_html=True)
arquivo_unico = st.file_uploader("Envie UM ato normativo por vez (PDF)", type=["pdf"], accept_multiple_files=False, key="uploader_unico")

if st.button("📖 Ler e Estruturar", type="primary", use_container_width=True, disabled=not arquivo_unico):
    if not api_key:
        st.error("⚠️ Insira sua chave da API nas configurações.")
    else:
        with st.spinner("⚡ Lendo e estruturando o ato no padrão do sistema..."):
            try:
                doc = estruturar_documento_unico(arquivo_unico, api_key.strip(), provedor_escolhido)
                st.session_state.ato_avulso = doc
                st.session_state.ato_avulso_original = copy.deepcopy(doc)
                st.session_state.resultado_cascata = None
                st.session_state.resultado_cascata_original = None
                st.success("✨ Ato lido e estruturado no editor abaixo.")
            except Exception as e:
                st.error(f"❌ Ocorreu um erro: {str(e)}")

if st.session_state.ato_avulso:
    st.markdown("---")
    st.markdown(f"## 📝 {st.session_state.ato_avulso['norma_base']['nome_padronizado']}")
    renderizar_editor_documento(st.session_state.ato_avulso, "avulso")

    col_salvar, col_analisar = st.columns(2)
    with col_salvar:
        if st.button("💾 Salvar Ato", use_container_width=True, key="btn_salvar_avulso"):
            ok, erro = salvar_ato_avulso(st.session_state.ato_avulso)
            if ok: st.success("Ato salvo no banco de dados.")
            else: st.error(f"❌ Falha ao salvar: {erro}")
    with col_analisar:
        if st.button("🔍 Análise com Atos Cadastrados", type="primary", use_container_width=True, key="btn_analisar_avulso"):
            if not api_key:
                st.error("⚠️ Insira sua chave da API nas configurações.")
            else:
                with st.spinner("⚡ Consultando o banco por relações de derivação e aplicando a cascata..."):
                    try:
                        resultado, mensagens = analisar_ato_avulso_contra_cadastrados(st.session_state.ato_avulso, api_key.strip(), provedor_escolhido)
                        for tipo_msg, texto_msg in mensagens:
                            (st.info if tipo_msg == "info" else st.warning)(texto_msg)
                        if resultado:
                            st.session_state.resultado_cascata = resultado
                            st.session_state.resultado_cascata_original = copy.deepcopy(resultado)
                    except Exception as e:
                        st.error(f"❌ Ocorreu um erro: {str(e)}")

    renderizar_exportacao(st.session_state.ato_avulso, "avulso")

if st.session_state.resultado_cascata:
    st.markdown("---")
    cons = st.session_state.resultado_cascata
    nomes_alteradoras = [a['nome_padronizado'] for a in cons.get('normas_alteradoras', [])]
    st.markdown(f"## 📊 Cascata: **{cons['norma_base']['nome_padronizado']}** alterada por **{' e '.join(nomes_alteradoras) if nomes_alteradoras else '—'}**")
    renderizar_editor_documento(cons, "cascata")

    if st.button("💾 Salvar Cascata Inteira no Banco de Dados", use_container_width=True, key="btn_salvar_cascata"):
        if salvar_cascata_e_vincular(cons): st.success("Cascata salva e atos vinculados no banco!")
        else: st.error("❌ Falha ao salvar a cascata.")

    renderizar_exportacao(cons, "cascata")

    if st.button("🔄 Nova Leitura", type="secondary"):
        for chave in ["ato_avulso", "ato_avulso_original", "resultado_cascata", "resultado_cascata_original"]:
