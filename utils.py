...
def init_supabase() -> Optional[Client]:
    try:
        url = st_secrets_getter("supabase", "url")
        key = st_secrets_getter("supabase", "key")
        if url and key:
...
def st_secrets_getter(section, key):
...
# ----------------- MOTOR DE IA E CONSULTA HISTÓRICA -----------------
def buscar_historico_supabase(nome_base, ano_base):
    """Consulta o Supabase para verificar se a portaria base já possui alterações salvas."""
...
def analisar_lote_arquivos(arquivos, api_key):
    ...
    try:
        ...
        2. Extraia o ano exato de criação/publicação da portaria base e da alteradora (`ano_portaria_base` e `ano_portaria_alteradora`).
        ...
        # CRUZAMENTO INTELIGENTE COM O HISTÓRICO DO SUPABASE:
        # Se a portaria base já tiver alterações antigas salvas no banco, injetamos essa informação
        for cons in dados.get("consolidacoes_geradas", []):
            ...
            if historico_antigo:
                ...
                # Atualiza o cabeçalho complemento para refletir a cadeia cumulativa
...
# ----------------- PARSERS E GERADORES DE ARQUIVO -----------------
