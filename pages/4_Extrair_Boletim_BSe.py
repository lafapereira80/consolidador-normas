...
def extrair_texto_boletim_estruturado(pdf_bytes: bytes) -> str:
    ...
    for page in doc:
        ...
        try:
            # 1. Estratégia principal: Tabelas com bordas (Sempre confiável)
            tf_lines = page.find_tables(strategy="lines")
            if tf_lines.tables:
                tabelas_validas.extend(tf_lines.tables)
                
            # 2. Estratégia secundária: Tabelas alinhadas (Análise Crítica Anti-Falso-Positivo)
            ...
            if tf_text.tables:
                for tb in tf_text.tables:
                    if tb.col_count > 1 and tb.row_count > 1:
                        ...
                        # Filtro 1: A tabela não pode conter a abertura de uma Portaria/Ato
                        if re.search(r'(Portaria|RESOLUÇÃO|ATO)\s+n[º°o\.]?\s*\d+', texto_tabela, re.IGNORECASE):
                            continue
                        
                        # Filtro 2: A tabela não pode conter artigos, parágrafos ou incisos (é um texto normativo)
                        if re.search(r'(Art\.\s*\d+|§\s*\d+|Parágrafo único)', texto_tabela, re.IGNORECASE):
                            continue
                            
                        # Filtro 3: Não pode ter células extremamente longas (mais de 250 caracteres = parágrafo)
                        if any(len(str(c)) > 250 for linha in linhas for c in linha if c):
                            continue
        ...
        for b in blocks:
            ...
            if any(bloco_rect.intersect(tb).get_area() > (bloco_rect.get_area() * 0.4) for tb in tabelas_bbox):
                continue
            ...
            for line in b.get("lines", []):
                ...
                for span in line.get("spans", []):
                    ...
                    if bool(flags & 2**4) or "bold" in font_name or "black" in font_name:
            ...
            if is_centered:
                align_class, indent_style = "ql-align-center", ""
            elif is_right:
                align_class, indent_style = "ql-align-right", ""
            else:
...
def extrair_atos_normativos_html(texto_html: str) -> List[Dict[str, str]]:
    # Divide o documento inteiro pelas tags principais geradas para podermos varrer bloco a bloco
    ...
    def is_inicio_ato(elem_html: str) -> bool:
        ...
        # Procura a assinatura clássica do ato (Portaria nº X)
        match = re.search(r'(Portaria|RESOLUÇÃO|ATO)\s+n[º°o\.]?\s*\d+', texto_puro, re.IGNORECASE)
        if not match:
            return False
            
        start_idx = match.start()
        # Se a palavra Portaria for literalmente a primeira coisa do bloco
        if start_idx < 5:
            return True
            
        # Se houve algo antes, validamos se foi apenas o cabeçalho institucional (Ex: ATOS DA PROCURADORIA...)
        prefixo = texto_puro[:start_idx].strip().upper()
        prefixos_permitidos = ["ATOS", "PROCURADORIA", "DIRETORIA", "GABINETE", "MINISTÉRIO", "MPM"]
        
        if len(prefixo) < 150:
            if any(p in prefixo for p in prefixos_permitidos):
                return True
        return False
    ...
    for elem in elementos:
        ...
        # O elemento atual sinaliza o nascimento de uma nova portaria/ato?
    ...
    for idx, corpo in enumerate(atos):
        ...
        # Tenta extrair a linha exata do Ato para o Título (ex: Portaria nº 159/PGJM...)
        ...
        cargo_autoridade = "Procurador-Geral de Justiça Militar" if "/PGJM" in primeira_linha.upper() else "Diretor-Geral do Ministério Público Militar" if "/DG" in primeira_linha.upper() else ""
...
if arquivo_bse is not None:
    with st.spinner("⚡ Lendo e analisando o Boletim de Serviço (processando tabelas, parágrafos e assinaturas)..."):
        texto_boletim_html = extrair_texto_boletim_estruturado(arquivo_bse.getvalue())
