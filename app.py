...
if st.button("🚀 Iniciar Análise Autopilot", type="primary", use_container_width=True):
    if not api_key: st.error("⚠️ Insira sua chave da API nas configurações.")
        ...
        with st.spinner("⚡ Executando OCR Estrutural e Consulta ao Histórico de Aprendizado..."):
            ...
            try:
                if fluxo_inteligente:
                    resultado = analisar_lote_arquivos(
                        arquivos_enviados,
                        api_key.strip(),
                        provedor_escolhido,
                ...
                st.session_state.dados_originais_ia = copy.deepcopy(st.session_state.dados_processados) if st.session_state.dados_processados else None
...
if st.session_state.confirmacao_pendente:
    ...
    for d in derivacoes:
        st.markdown(f"- **Arquivo:** {d['nome_arquivo_upload']} deriva de {d['ato_base_referenciado_tipo']} {d['ato_base_referenciado_numero']} ({d['nome_base']})")
