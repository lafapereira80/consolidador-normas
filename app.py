...
3. TABELAS: quando o dispositivo contiver uma tabela no texto de origem, transcreva TODAS as linhas e colunas, e inclua a tabela com fidelidade absoluta em 'tabela_alterada' e
   'tabela_consolidada' (uma lista de listas, uma sublista por linha, mantendo a ordem exata de colunas).
   NUNCA descreva a tabela em prosa, NUNCA a omita, e NUNCA resuma seu conteúdo — reproduza célula a célula, inclua a tabela, faça a tabela.
4. CRITÉRIO RIGOROSO DE ALTERAÇÃO E REVOGAÇÃO: 
   - Analise cirurgicamente o texto original do ato base em confronto com a ou as portarias alteradoras.
   - Na versão ALTERADA (`texto_principal_alterada`), todo dispositivo, parágrafo, inciso ou alínea que sofreu alteração de mérito ou revogação expressa DEVE obrigatoriamente, após a identificação do Art, Inciso, Paragrafo, paragrafo único e etc, aparecer envelopado com a tag exata: `<strike><font color="red">texto antigo alterado ou revogado</font></strike>` seguido imediatamente, na linha abaixo, pelo texto novo vigente (quando houver nova redação). Se o dispositivo foi apenas revogado, ele permanece taxado em vermelho com a tag citada.
   ...
   - Na versão CONSOLIDADA (`texto_principal_consolidada`) de um dispositivo ALTERADO (nova redação), exiba o número/identificador do dispositivo (ex.: "Art. 5º", "§ 3º", "Inciso II") seguido da ALTERADA PELA por extenso, sem riscos — nunca a redação antiga.
