...
# Estrutura Pydantic para garantir formato JSON limpo
class Dispositivo(BaseModel):
    tipo: str = Field(description="Tipo do item, ex: 'capitulo', 'artigo', 'paragrafo'")
    texto_alterada: str = Field(description="Texto para a Versão Alterada. Se foi alterado/revogado, coloque a versão antiga tachada em HTML (<font color='red'><strike>...</strike></font>) seguida da nova redação com nota remissiva.")
...
def gerar_pdf_dinamico(titulo_versao, dados_json, tipo_versao):
    ...
    # 2. Brasão (Lido diretamente da pasta do GitHub/Streamlit)
    ...
    if os.path.exists(caminho_imagem):
        try:
            ...
            pass # Se o arquivo estiver corrompido, apenas ignora para não travar o PDF
...
if st.button("🚀 Processar Dinamicamente com IA e Gerar PDFs", type="primary"):
    if not api_key:
        ...
        with st.spinner("Lendo os PDFs, cruzando os atos e estruturando os dados dinamicamente... (Isso pode levar alguns segundos)"):
            try:
