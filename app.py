...
# Configuração moderna de navegação do Streamlit
pg = st.navigation([
    st.Page(lambda: run_autopilot(), title="Autopilot", icon="⚖️"),
    st.Page(lambda: historico.render_historico(), title="Histórico", icon="🗄️")
], position="sidebar")
...
def run_autopilot():
    ...
    with col_link:
        ...
        # Link nativo seguro para a página de histórico
        st.page_link(pg.pages[1], label="Ver Histórico", icon="🗄️")
...
# Executa a página ativa selecionada no menu
pg.run()
